...
describe('Authentication Middleware', () => {
  ...
  describe('devBypassMiddleware - CRITICAL SECURITY VULNERABILITY', () => {
    let originalNodeEnv: string | undefined;
    let originalDevBypassAuth: string | undefined;

    beforeEach(() => {
      originalNodeEnv = process.env.NODE_ENV;
      originalDevBypassAuth = process.env.DEV_BYPASS_AUTH;
    });
    ...
    describe('when DEV_BYPASS_AUTH is enabled in development', () => {
      it('should NOT bypass authentication - security fix applied', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        process.env.DEV_BYPASS_AUTH = 'true';
        mockRequest.path = '/test-endpoint';

        const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation();
        
        // Mock adminAuthMiddleware to track if it's called
        const mockAdminAuth = jest.fn((req, res, next) => next());
        jest.doMock('../auth', () => ({
          ...jest.requireActual('../auth'),
          adminAuthMiddleware: mockAdminAuth
        }));

        // Act
        devBypassMiddleware(mockRequest as Request, mockResponse as Response, mockNext);

        // Assert - Should log security error instead of bypassing
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          '🚨 SECURITY ERROR: devBypassMiddleware called but auth bypass is disabled for security'
        );
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          'Use proper admin authentication instead'
        );

        // Should NOT set mock admin user
      ...
      it('should always use real authentication - no security bypass allowed', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        process.env.DEV_BYPASS_AUTH = 'true';
        mockRequest.path = '/admin/dangerous-endpoint';
        ...
        // Assert - Should always enforce real authentication
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          '🚨 SECURITY ERROR: devBypassMiddleware called but auth bypass is disabled for security'
        );
    ...
    describe('when DEV_BYPASS_AUTH is disabled in development', () => {
      it('should use real admin authentication', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
      ...
      it('should fail authentication when token is invalid', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        process.env.DEV_BYPASS_AUTH = 'false';
    ...
    describe('when DEV_BYPASS_AUTH is undefined in development', () => {
      it('should use real admin authentication (default behavior)', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        delete process.env.DEV_BYPASS_AUTH; // Undefined
    ...
    describe('SECURITY VULNERABILITY: production environment scenarios', () => {
      it('should NEVER bypass authentication in production even if DEV_BYPASS_AUTH is true', () => {
        // Arrange
        process.env.NODE_ENV = 'production';
        process.env.DEV_BYPASS_AUTH = 'true'; // This should be ignored in production
        
        mockRequest.headers = {}; // No auth token
        mockedJWTUtils.extractTokenFromHeader.mockReturnValue(null);
        ...
        // Assert
        expect(consoleSpy).not.toHaveBeenCalled(); // No bypass warning should be logged
        expect(mockRequest.admin).toBeUndefined(); // No mock admin should be set
        expect(mockStatus).toHaveBeenCalledWith(401); // Should fail authentication
        expect(mockJson).toHaveBeenCalledWith({
          success: false,
          error: 'Admin authentication required',
          code: 'NO_ADMIN_TOKEN'
      ...
      it('should use real admin authentication in production', () => {
        // Arrange
        process.env.NODE_ENV = 'production';
    ...
    describe('SECURITY VULNERABILITY: test environment scenarios', () => {
      it('should NEVER bypass authentication in test environment', () => {
        // Arrange
        process.env.NODE_ENV = 'test';
        process.env.DEV_BYPASS_AUTH = 'true'; // This should be ignored in test
    ...
    describe('edge cases for bypass mechanism', () => {
      it('should handle case-sensitive environment variable', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        process.env.DEV_BYPASS_AUTH = 'TRUE'; // Wrong case
      ...
      it('should handle various truthy strings correctly', () => {
        // Arrange
        process.env.NODE_ENV = 'development';
        process.env.DEV_BYPASS_AUTH = '1'; // Not exactly 'true'
